...
public:
  ...
  void solve()
  {
    ...
    for (int n = 0; n < Nt; n++)
    {
      ...
      b[N - 1] = a_p - a_e;
      d[0] -= 2 * a_w * 100;
      int T2=(T_inf-(2*k*T[N-1])/(h*dx))/(1-2*k/(h*dx));
      d[N - 1] = T[N-1]+del_p-2*T2*a_e; // modifying the boundary condition for the convective heat transfer from the tip
  ...
  // Function to display results and save them to a file
  void displayResults() 
  {
    // Create and open a text file
    ofstream outFile("ConvectionResults.txt");
    ...
    if (outFile.is_open())
    {
      ...
      cout << "Results have been saved to ConvectionResults.txt" << endl;
    }
    else
    {
...
// The main function should be outside of the class definition
int main()
{
